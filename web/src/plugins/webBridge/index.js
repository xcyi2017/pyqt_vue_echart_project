...
class WebBridge {
    constructor(qtObjName, callback = _ => { }) {
        ...
        new QWebChannel(window.qt.webChannelTransport, (channel) => {
          ...
          let QtServer = channel.objects[qtObjName];
...
class ApiClient {
    constructor(qtObjName, sendActionName, listenEventName, callback = _ => { }) {
        ...
        this.sendActionName = sendActionName // 槽函数
        this.listenEventName = listenEventName; // 信号
        this.callbackList = {};
        this.callbackId = 0;
    }
    ...
    send(request) {
        return new Promise((resolve, reject) => {
            ...
            console.log("send_request", request)
            this.webBridge.send({
    ...
    addResponseListener() {
        this.webBridge.on(this.listenEventName, (responseStr) => {
            // console.log("responseStr:",responseStr.slice(2,-1));
            let response = JSON.parse(responseStr.slice(2,-1));
